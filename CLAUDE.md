...
**Probes run sequentially** (`ThreadPoolExecutor(max_workers=1)`): many endpoints fan out to different EKS pods but share one pgbouncer/Postgres, so concurrent probing would burst the shared pool. A healthy run takes ~40 s; the 5-min schedule leaves no overlap.

**The per-probe timeouts must sum to less than the Lambda timeout.** Sequential probing means the worst case — every endpoint hanging until its socket timeout — costs their *sum*, not their max. Overrun it and the run is killed mid-probe having written **nothing**: no `status.json`, no uptime slot, and no `probe_latency` lines either, since `log_response_times()` only runs after every probe returns. The page goes stale exactly during the outage it exists to report, and the latency log has a hole on precisely the runs worth investigating. This was real: the config summed to **575 s** against a 240 s timeout, and **72 of 2073 runs** in the week to 13/08/2026 died at the wall. Fixed on 13/08/2026 by recalibrating the timeouts to a 252 s budget and raising the Lambda to `timeout = 270`; `test_real_config_timeout_budget_fits_the_lambda_timeout` reads the ceiling straight out of `infrastructure/lambda.tf` so the two can't drift apart. Adding a probe now spends from ~18 s of slack and will eventually force a real trade-off rather than silently eating the margin. Never raise the Lambda timeout to or past **300 s** — that's the schedule interval, and overlapping runs would hit the shared pool concurrently.

**Calibrate thresholds from CloudWatch, never from the local dry-run.** The dry-run probes from a workstation rather than from the Lambda in `us-east-1` and reads high by a large per-request margin — `api-ztf-dr-lightcurve` measured 1025 ms locally against a 25 ms Lambda median (~40x), and the static frontends ~150 ms against ~16 ms. Use the dry-run to confirm a config change is *valid* and an endpoint answers as expected; use `filter metric = "probe_latency" | stats ... by id` in Logs Insights to set numbers.
...
- **Latency baseline ~1.0 s** for all three ADQL probes (indexed ZTF lookup, indexed multi-survey lookup, `CONTAINS`/`CIRCLE` cone search — the spatial path is no more expensive than the keyed one), measured over 12 days to 05/08/2026: p50 0.98-0.99 s, p95 1.08-1.76 s, p99 1.87-3.33 s. The body is tight but the tail is **not** Gaussian — p99.9 reaches 7.1-7.7 s, so the single-replica burst concern is real even though σ on the body is small. Their original `6000 / 15000 / 20 s` guess was **recalibrated away** on 05/08/2026 onto the then-global `4000 / 10000 / 15 s`, replacing an `latency_outage_ms` of 15 s that the observed max of 8.6 s made **unreachable**. They carry explicit `4000 / 8000 / 8 s` overrides again as of 13/08/2026 — not a reversal of that reasoning but a consequence of the globals tightening to `3000 / 4000 / 5 s`, which sits under TAP's non-Gaussian p99.9 tail and would flag the VPN-floor burst as an outage. The override keeps the same intent (degraded at ~2x the p99 body, outage above the observed max) at a timeout the run's budget can afford.
