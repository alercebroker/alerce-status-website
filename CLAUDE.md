...
- **Latency baseline ~1.5 s with σ ≈ 0.03 s** for all three ADQL probes (indexed ZTF lookup, indexed multi-survey lookup, `CONTAINS`/`CIRCLE` cone search — the spatial path is no more expensive than the keyed one). Their `6000 / 15000 / 20 s` overrides are set for burst tails on a single replica, not from observed tails; **recalibrate from the private `metric=probe_latency` log** once there's a week of history rather than leaving them at a guess.
